...
from flask_cors import CORS  # Импорт CORS из flask_cors
import sqlite3
import os
...
def save_game_result():
    data = request.json
    try:
        user_id = int(data.get('user_id'))  # Преобразуем в число
    except:
    ...
    try:
        if result:
            current_best = result[0] or 0
            current_nft = result[1] or 0
